...
| **M3-2** | 设置页（F-06）：✅ **双 Tab 设置页已实现（2026-08-14，v3 预览 `.codex/previews/settings-tabs-*-*.png` 获用户批准后落地）**：① 全宽控制台布局（与其他页面一致，App.vue 设置页进入 operations workspace）；② 双 Tab：**应用设置**（系统状态/远程连接/数据与队列/使用时间口径三列/配置项 unsupported 清单 + 说明，全部只读复用 capability report + usage summary）与 **系统信息**（appd 以固定 `--structure` 运行本机 fastfetch 2.67 `--format json`，6s 超时，16 个分区归一化为成对网格事实行；fastfetch 缺失 → `unsupported + fastfetch_not_found`，失败可重试、刷新失败保留旧数据）。**新契约（用户明确授权的唯一新增项）**：`system_info` IPC 请求（crates/systeminfo 新 crate + ipc `SystemInfo` 请求/响应 + appd `SystemInfoProvider` + Tauri command `system_info` 只读代理，无新权限）。验证：systeminfo 10/10、ipc 全量 64/64（含 systeminfo_roundtrip 3/3）、desktop-ui 224/224、typecheck/build 通过、隔离窗口 AT-SPI 复核双 Tab 全部 landmark 与 16 分区。**后续契约工作计划（需另行授权）**：①保留期可配置（usage `RetentionPolicy`、notes purge/prune 已存在但为固定默认值，需新增 settings 读写 command + appd 持久化 + 热生效）；②采集周期可配置（telemetry 采样间隔现为编译期常量）；③远程连接默认值（表单预填策略）；④通知/快捷键按产品边界保持 unsupported | 产品 owner | ✅ 已完成（2026-08-14）；契约项需新授权后再立项（Preview-first + 契约证据） |
